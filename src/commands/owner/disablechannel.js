...
module.exports = {
    ...
    run: async ({client, message, args}) => {
        ...
        const channelId = message.mentions?.channels?.first()?.id ?? message.guild?.channels?.cache?.get(args[1]) ?? await client.channels?.fetch(args[1]).then(async (channels) => {
            return channels?.id
        });
        if (!channelId) return message.channel.send('Please provide a valid user to blacklist!');

        if (query === 'add' || query === 'create') {
            await BlacklistChannel.findOne({channelId: channelId}, async (error, data) => {
            ...
            await BlacklistChannel.findOneAndDelete({channelId: channelId}, async (error, data) => {
                if (error) return null;
                if (data) {
