	...
	run: async ({ client, message, args }) => {
		...
		if (!member) {
			return message.channel.send({
				embeds: [new MessageEmbed().setColor('RED').setDescription(`You need to mention a member to timeout!`)],
			});
		}
		args.shift();
		let duration = null;
		...
		if (!member.kickable) {
			return message.channel.send({
				embeds: [
					new MessageEmbed()
						.setColor('RED')
						.setDescription(`I can't timeout this member because I don't have the permissions to do so!`),
				],
			});
		}
		...
		if (message.member.roles.highest.comparePositionTo(member.roles.highest) < 1) {
			return message.channel.send({
				embeds: [
					new MessageEmbed()
						.setColor('RED')
						.setDescription(`You can't timeout this member because you don't have the permissions to do so!`),
				],
			});
		}
		...
		if (duration) {
			...
			message.channel.send({
				embeds: [new MessageEmbed().setColor('RED').setDescription(`You need to specify a duration for the timeout!`)],
			});
		}
	},
};
