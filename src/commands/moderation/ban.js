	...
	run: async ({ client, message, args }) => {
		...
		if (!member) {
			return message.channel.send(
				new MessageEmbed().setColor('RED').setDescription(`You need to mention a member to ban!`)
			);
		}
		...
		if (!member.bannable) {
			return message.reply({
				embeds: [
					new MessageEmbed()
						.setColor('RED')
						.setDescription(`I can't ban this member because I don't have the permissions to do so!`),
				],
			});
		}
		...
		if (message.member.roles.highest.comparePositionTo(member.roles.highest) < 1) {
			return message.reply({
				embeds: [
					new MessageEmbed()
						.setColor('RED')
						.setDescription(`You can't ban this member because you don't have the permissions to do so!`),
				],
			});
		}
