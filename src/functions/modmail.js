...
client.on("messageCreate", async message => {
  ...
  const staff = config.modmail.staffID
	const guild = await client.guilds.cache.get(config.mainGuildID); // Getting the server
	const category = config.modmail.categoryID; // Modmail category
	const logsChannel = guild.channels.cache.get(config.modmail.logsChannelID); // modmail_logs channel
    ...
    if(message.channel.id === config.modmail.discussion) return
		...
		if (message.content === "-" + "close") {
			...
			setTimeout(() => {
				...
				message.channel
					...
					.then(ch => {
						guild.channels.cache.get(config.modmail.logsChannelID).send({
							// sending a log
							embeds: [
								new MessageEmbed()
