...
module.exports = {
  ...
  async execute(oldPresence, newPresence) {
    ...
    if (
      newPresence.guild.id === "952514062904860692" ||
      newPresence.guild.id === "697495719816462436"
    ) {
      ...
      if (newState === null && newPresence.member.roles.cache.get(chad))
        return newPresence.member.roles.remove(chad);
      if (!oldPresence.activities[0] && !newPresence.activities[0]) return;

      if (
        newPresence.status === "offline" &&
        newPresence.member.roles.cache.get(chad)
      )
        return newPresence.member.roles.remove(chad);
      ...
      if (
        newPresence.member.roles.cache.get(chad) &&
        !newState.includes("discord.gg/igp")
      ) {
        newPresence.member.roles.remove(chad);
      }
      if (
        newPresence.state === "offline" &&
        newPresence.member?.roles.cache.get(chad)
      ) {
        newPresence.member.roles.remove(chad);
      }
