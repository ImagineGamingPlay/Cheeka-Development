...
module.exports = {
	...
	async execute(interaction, client) {
		...
		if (!interaction.customId.startsWith("a") && !interaction.customId.startsWith("d")) return;
		...
		let tag = Array.from(tagsCache.values()).find(tag => tag._id?.valueOf() === id);
		...
		if (!interaction.message.member.permissions.has(Permissions.FLAGS.KICK_MEMBERS)) {
		...
		if (enable === "a") {
			...
			let embed = new MessageEmbed()
				...
				.setDescription(`Tag **${tag.name}** was accepted by <@${interaction.member.id}>`)
			...
			let embed = new MessageEmbed()
				.setColor("RED")
				.setTitle("Tag Denied")
				.setDescription(`Tag **${tag.name}** was denied by <@${interaction.member.id}>`)
				.addField("Tag", tag.name)
				.setTimestamp();
			await interaction.editReply({
				embeds: [embed],
